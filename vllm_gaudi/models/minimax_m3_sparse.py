...
class MiniMaxM3LightningIndexer:
    ...
    def select(
        ...
        if self.local_blocks > 0:
            ...
            forced |= (block_ids[None, :] >= lo[:, None]) & (
                block_ids[None, :] <= q_block[:, None]
            )
        ...
        out = torch.full(
            (n_heads, t_len, self.topk_blocks), -1, dtype=torch.int64,
...
def minimax_m3_block_sparse_attention(
    ...
    if topk_block_ids is not None:
        ...
        allow_block = torch.zeros(
            n_kv_heads, t_len, n_blocks, dtype=torch.bool, device=query.device
        )

...
class MiniMaxVLAttention(nn.Module):
    ...
    def _apply_rope_partial(
        ...
        # The HPU/native ``ApplyRotaryEmb`` (forward_static) splits the FULL
        # head_dim in half, so it only supports full rotary
        # (rot_dim == head_dim). MiniMax-M3 vision uses PARTIAL rotary
        # (rot_dim = 2 * cos.shape[-1] < head_dim), so cos/sin are narrower than
        # head_dim//2 and the full split would broadcast-mismatch (the CUDA
        # flash-attn path handles this internally, the native path does not).
        # Rotate only the first ``rot_dim`` dims and pass the tail through.
        rot_dim = cos.shape[-1] * 2
        if rot_dim >= qk.shape[-1]:
            return self.apply_rotary_emb(qk, cos, sin)
        qk_rot = self.apply_rotary_emb(qk[..., :rot_dim].contiguous(), cos, sin)
        return torch.cat((qk_rot, qk[..., rot_dim:]), dim=-1)
    ...
    def _apply_rotary_emb(
        ...
        if not current_platform.is_rocm() or rotary_segment_lengths is None:
            return self._apply_rope_partial(qk_reshaped, rotary_cos, rotary_sin)
        ...
        return torch.cat(
            [
                self._apply_rope_partial(qk_s, cos_s, sin_s)

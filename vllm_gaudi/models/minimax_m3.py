...
from vllm.model_executor.layers.fused_moe import (
    FusedMoE, fused_moe_make_expert_params_mapping)
...
from vllm_gaudi.models.minimax_m3_mm import (MiniMaxM3VLDummyInputsBuilder,
                                             MiniMaxM3VLMultiModalProcessor,
                                             MiniMaxM3VLProcessingInfo)
from vllm_gaudi.models.minimax_m3_sparse import (GemmaRMSNorm,
                                                 MiniMaxM3LightningIndexer,
                                                 swiglu_oai)
...
class HpuMiniMaxM3MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = swiglu_oai(gate_up, self.swiglu_alpha, self.swiglu_beta,
                       self.swiglu_limit)
...
class HpuMiniMaxM3MoE(nn.Module):
    ...
    def __init__(
        ...
        if self.use_routing_bias:
            ...
            self.e_score_correction_bias.weight_loader = (
                HpuMiniMaxM3MoE.ebias_weight_loader)
        else:
    ...
    @staticmethod
    def ebias_weight_loader(param: nn.Parameter,
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        routed = self.experts(hidden_states=hidden_states,
        ...
        for name, loaded_weight in weights:
            ...
            for param_name, weight_name, shard_id in stacked_params_mapping:
                if weight_name not in name:
                    continue
                if ("block_sparse_moe.experts." in name) and (
...
class HpuMiniMaxM3SparseForCausalLM(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.model = HpuMiniMaxM3Model(vllm_config=vllm_config,
...
class HpuMiniMaxM3SparseForConditionalGeneration(nn.Module, SupportsMultiModal):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.use_data_parallel = (
            self.multimodal_config.mm_encoder_tp_mode == "data")
        ...
        self.make_empty_intermediate_tensors = getattr(
            self.language_model, "make_empty_intermediate_tensors", None)
    ...
    def _process_image_input(self, image_input: dict) -> tuple[torch.Tensor, ...]:
        ...
        if self.use_data_parallel:
            return run_dp_sharded_mrope_vision_model(self.vision_tower,
                                                     ...
                                                     grid_thw.tolist(),
                                                     rope_type="rope_3d")
        image_embeds = self.vision_tower(pixel_values=pixel_values,
    ...
    def _process_video_input(self, video_input: dict) -> tuple[torch.Tensor, ...]:
        ...
        if self.use_data_parallel:
            return run_dp_sharded_mrope_vision_model(self.vision_tower,
                                                     ...
                                                     grid_thw.tolist(),
                                                     rope_type="rope_3d")
        video_embeds = self.vision_tower(pixel_values=pixel_values,
                                         grid_thw=grid_thw.tolist())
        merge_size = self.vision_tower.spatial_merge_size
        sizes = (grid_thw.prod(-1) // (merge_size * merge_size)).tolist()
